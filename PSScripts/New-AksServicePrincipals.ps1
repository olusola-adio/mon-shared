...
If executed from Azure DevOps this will be the default subscription of the Azure DevOps Service Principal.  The name should be in the format mon-<env>-shared-aks-svc.

.PARAMETER AksAdClientApplicationName
The name of the AAD Application registration that will be granted permissions user_impersonation on the AksAdServerApplicationName app registration.  The name should be in the format mon-<env>-shared-aks-client.
...
The permissions granted will be Directory.Read.All and User.Read.  The name should be in the format mon-<env>-shared-aks-api.
...
# Validates that the AksServicePrincipal has only been assigned 2 roles (One for mon-<env>-shared-rg, the other for mon-<env>-shared-aksnodes-rg).
...
if ($Logs -match "Registering service principal ...") {
    ...
    Write-Verbose "Service Principal creation detected in logs, manual steps required, see mon-shared README.md"
    throw "Service Principal permissions needs approving and variables may need to be created or updated in Azure DevOps, see mon-shared README.md"
